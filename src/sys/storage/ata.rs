...
use core::{default, ops::{Index, IndexMut}, slice::SliceIndex, hash::Hasher, fmt::{Debug}};

use alloc::string::String;
use spin::Spin;
use x86_64::{instructions::port::{Port, PortReadOnly, PortWriteOnly}, structures::port::PortWrite};
use bit_field::BitField;

use crate::{KResult, sys::{timer::{self, uptime, sleep_ticks}, self, terminal::Spinner}, print, log};

use super::sector::{Sector, self};
...
impl Bus {
    pub fn new(io_base: u16, ctrl_base: u16, irq: u8) -> Bus {
        Bus {
    ...
    fn buzy_loop(&mut self) {
        ...
        let start = uptime();
        while self.is_busy() {
            if (uptime() - start) > 1.0 {self.reset()};
            crate::spin();
        }
    }
    ...
    fn setup(&mut self, drive: u8, block: u32) {
        ...
        unsafe {
            self.io_reg.drive.write(drive_id | ((block.get_bits(24..28) as u8) & 0x0F));
    ...
    fn identify_drive(&mut self, drive: u8) -> Option<[u16; 256]> {
        ...
        if self.lba_mid() != 0 || self.lba_high() != 0  { return None; }
...
pub unsafe fn copy(dest_drive: usize, src_drive: usize, start: u32, amount: usize) -> KResult<()> {
    ...
    log!("Copying Sectors...{} - ({:05}/{:05})\r", spinner.glyph(), prog, amount);
